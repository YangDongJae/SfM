...
class SFM_ORIGIN:
    ...
    def load_intrinsic(self):
        k_path = os.path.join(self.dir_path, "K.txt")
        if os.path.exists(k_path):
            try:
                with open(k_path, 'r') as f:
                    ...
                    for i, line in enumerate(lines[:3]):
                        ...
                        intrinsic[i, :len(values)] = values
                
                self.intrinsic = intrinsic
                print(f"Camera intrinsic matrix loaded from {k_path}")
                return True
            ...
            print(f"Warning: K.txt not found in {self.dir_path}")
            return False
    ...
    def set_dir_path(self, num_images, dir_path):
        ...
        if not self.load_intrinsic():
            print("Warning: Using default intrinsic matrix")
    
            self.intrinsic = np.array([[1000.0, 0.0, 500.0],
                                      [0.0, 1000.0, 500.0],
